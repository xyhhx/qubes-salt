...
Documenting the configuration of my Qubes OS workstation through config-as-code
with salt
...
>
> If you want to use it, feel free; but no promises anything works as expected.
> Feel free to open issues though!
...
<!-- rumdl-disable MD013 -->
-   In a guest VM, clone the repo into `/usr/local/src` (this avoids username
    conflicts later)
...
Make is used to interact with `qubesctl` and offers a few helper tasks to make
working with top files a bit easier:
...
-   Applying "batch targets" (i.e. `--all`, `--templates`, `--standalones`,
  `--apps`):
...
Bundling changes and lifting them to dom0 is also easily done with the available
Make tasks:
...
The following resources were very useful for learning Salt, especially in the
context of Qubes OS; and for getting inspiration about how to design my own
states (thanks!):
