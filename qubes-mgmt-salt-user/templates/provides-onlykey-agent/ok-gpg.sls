...
{{ download_github_release(
  download_url=ok_agent_pkg["download_url"],
  checksum=ok_agent_pkg["release"].checksum,
  output=tmpdir | path_join(ok_agent_pkg["release_asset"])
) }}
...
"{{ slsdotpath }}:: extract ok bins":
  archive.extracted:
    ...
    - name: "/usr/bin"
    - source: "{{ tmpdir | path_join(ok_agent_pkg["release_asset"]) }}"
